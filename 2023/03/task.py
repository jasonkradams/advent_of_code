    ...
    for x, line in enumerate(symbol_grid):
        for y, is_symbol in enumerate(line):
            if is_symbol:
                ...
                # ...
                total_task_1 += sum(
                    [task_1(x=cx, y=cy) for cx, cy in get_adjacent_cells(x=x, y=y)]
                )
                t2 = [task_2(x=cx, y=cy) for cx, cy in get_adjacent_cells(x=x, y=y)]
                # Task 2: This should be bubbled up into def task_2(); but this works and it's late.
